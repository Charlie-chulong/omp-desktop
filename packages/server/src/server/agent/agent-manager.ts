...
} from "@omp-desktop/protocol/agent-lifecycle";
import {
...
} from "@omp-desktop/protocol/agent-labels";
...
export class AgentManager {
  ...
  async setAgentModel(agentId: string, modelId: string | null): Promise<void> {
    ...
    if (agent.session.setModel) {
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (normalizedModelId && /^Model not found:/i.test(message)) {
          await this.reloadAgentSession(agentId, { model: normalizedModelId });
          return;
        }
        throw error;
      }
  ...
  async setAgentGoalObjective(agentId: string, objective: string): Promise<void> {
    const agent = this.requireAgent(agentId);
    if (!agent.session.setGoalObjective) {
      throw new Error("Agent session does not support editing goals");
    }
    await agent.session.setGoalObjective(objective);
    await this.drainSessionEvents(agentId);
    agent.config.featureValues = {
      ...agent.config.featureValues,
      goal_objective: objective.trim(),
    };
    this.touchUpdatedAt(agent);
    this.emitState(agent);
  }
  ...
  async controlAgentGoal(agentId: string, action: "start" | "pause" | "delete"): Promise<void> {
    const agent = this.requireAgent(agentId);
    if (!agent.session.controlGoal) {
      throw new Error("Agent session does not support controlling goals");
    }
    await agent.session.controlGoal(action);
    await this.drainSessionEvents(agentId);
    if (action === "delete") {
      agent.config.featureValues = {
        ...agent.config.featureValues,
