...
import { getOpenAgentTabLabel, PARENT_AGENT_ID_LABEL } from "@omp-desktop/protocol/agent-labels";
...
test("setAgentModel reloads a stale provider catalog when a new model is not found", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-model-refresh-"));

  class StaleCatalogSession extends TestAgentSession {
    async setModel(modelId: string | null): Promise<void> {
      throw new Error(`Model not found: ${modelId}`);
    }
  }

  class RefreshingCatalogClient extends TestAgentClient {
    resumedModel: string | null = null;

    override async createSession(config: AgentSessionConfig): Promise<AgentSession> {
      return new StaleCatalogSession(config);
    }
    ...
    override async resumeSession(
      ...
      this.resumedModel = config?.model ?? null;
      return new TestAgentSession({
        provider: this.provider,
        cwd: config?.cwd ?? workdir,
        model: config?.model,
  ...
  const client = new RefreshingCatalogClient();
  const manager = new AgentManager({
    clients: { codex: client },
    logger,
  ...
  try {
    const agent = await manager.createAgent(
      { provider: "codex", cwd: workdir, model: "gpt-old" },
      undefined,
      { workspaceId: undefined },
    );

    await manager.setAgentModel(agent.id, "mintcat/gpt-5.6-sol");

    expect(client.resumedModel).toBe("mintcat/gpt-5.6-sol");
    expect(manager.getAgent(agent.id)?.config.model).toBe("mintcat/gpt-5.6-sol");
  } finally {
    await manager.closeAgent("00000000-0000-4000-8000-000000000305");
    rmSync(workdir, { recursive: true, force: true });
  }
});
