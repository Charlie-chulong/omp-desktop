...
import { BUILTIN_PROVIDER_IDS } from "@omp-desktop/protocol/provider-manifest";
...
export const GLOBAL_PROVIDER_SNAPSHOT_KEY = "omp-desktop:global";

// OMP model discovery can be slow on a cold start. Operators can raise the
// ceiling through the environment without rebuilding.
...
export class ProviderSnapshotManager {
  ...
  async saveOmpProviderConfig(configYaml: string): Promise<OmpProviderManagement> {
    ...
    const client = this.ensureClient("omp", definition);
    if (!client.saveOmpProviderConfig) {
      throw new Error("OMP provider configuration is read-only");
    }
  ...
  async addOmpProvider(input: OmpCustomProviderInput): Promise<OmpProviderManagement> {
    ...
    const client = this.ensureClient("omp", definition);
    if (!client.addOmpProvider) throw new Error("Adding OMP providers is unavailable");
    return await client.addOmpProvider(input);
  }
  async removeOmpProvider(providerId: string): Promise<OmpProviderManagement> {
    const definition = this.providerRegistry.omp;
    if (!definition) throw new Error("OMP provider is not configured");
    const client = this.ensureClient("omp", definition);
    if (!client.removeOmpProvider) throw new Error("Removing OMP providers is unavailable");
    return await client.removeOmpProvider(providerId);
  }

  async getOmpInstallationStatus(options?: { checkForUpdates?: boolean }) {
    const definition = this.providerRegistry.omp;
    if (!definition) throw new Error("OMP provider is not configured");
    const client = this.ensureClient("omp", definition);
    if (!client.getOmpInstallationStatus) throw new Error("OMP installation status is unavailable");
  ...
  async logoutOmpProvider(providerId: string): Promise<OmpProviderManagement> {
    ...
    return await client.logoutOmpProvider(providerId);
  }
