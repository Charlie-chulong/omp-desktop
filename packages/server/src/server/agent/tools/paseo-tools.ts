...
} from "@omp-desktop/protocol/schedule/types";
...
export function createPaseoToolCatalog(options: PaseoToolHostDependencies): PaseoToolCatalog {
  ...
  const ProviderModelInputSchema = AgentProviderEnum.trim()
    .refine((value) => value.includes("/"), {
      message: "provider must be omp/model, for example omp/anthropic/claude-sonnet-4-6",
    })
    .refine(
      (value) => {
        try {
  ...
  const ProviderOrProviderModelInputSchema = AgentProviderEnum.trim()
    ...
    .refine(
      ...
      { message: "provider must be omp or omp/model" },
    );
  ...
  const commonCreateAgentFields = {
    ...
    provider: ProviderModelInputSchema.describe(
      "Required OMP model pair, for example omp/anthropic/claude-sonnet-4-6.",
    ),
  ...
  const inspectProviderInputSchema = {
    provider: ProviderOrProviderModelInputSchema.describe(
      "OMP provider ID, optionally with an OMP model ID.",
    ),
    cwd: z
      .string()
      .optional()
  ...
  registerTool(
    "image_gen",
    {
      ...
      inputSchema: {
        ...
        quality: z
          ...
          .default("medium")
          .describe(
            "Use medium for ordinary requests. High can take several minutes and should only be used when the user explicitly asks for maximum quality.",
          ),
        background: z.enum(IMAGE_GENERATION_BACKGROUNDS).optional().default("auto"),
        outputFormat: z.enum(IMAGE_GENERATION_OUTPUT_FORMATS).optional().default("png"),
      },
      outputSchema: {
        prompt: z.string(),
        model: z.string(),
        ...
        background: z.enum(IMAGE_GENERATION_BACKGROUNDS),
    ...
    async (args, context) => {
      if (!callerAgentId) {
        throw new Error("image_gen is only available to agent-scoped tool sessions");
      }
      if (!options.imageGenerationService) {
      ...
      const progressTimer = setInterval(() => {
        ...
        context.sendUpdate?.({
          content: [
            {
              type: "text",
              text: `Waiting for the image provider (${elapsedSeconds}s elapsed).`,
            },
          ],
      ...
      return {
        content: [
          {
            type: "text",
            text: `Generated image saved to ${generated.filePath}. The desktop renders it automatically.`,
          },
        ],
    ...
    "create_schedule",
    {
      ...
      inputSchema: {
        ...
        provider: (callerAgentId ? AgentProviderEnum.optional() : AgentProviderEnum).describe(
          "OMP provider or omp/model. Defaults to the caller's OMP model in an agent-scoped session.",
        ),
        cwd: z.string().optional(),
