...
class FakeAgentConfigOperations implements AgentConfigOperations {
  ...
  readonly goalCalls: Array<{ agentId: string; objective: string }> = [];
  readonly goalControlCalls: Array<{
    agentId: string;
  ...
  async setGoalObjective(agentId: string, objective: string): Promise<void> {
    this.goalCalls.push({ agentId, objective });
    this.callLog.push("goal");
    if (this.failWith) throw this.failWith;
  }

  async controlGoal(agentId: string, action: "start" | "pause" | "delete"): Promise<void> {
    this.goalControlCalls.push({ agentId, action });
    this.callLog.push(`goal:${action}`);
    if (this.failWith) throw this.failWith;
  }
...
describe("AgentConfigSession", () => {
  ...
  test("set goal objective: forwards the objective and emits an accepted response", async () => {
    const { subsystem, emitted, operations } = makeSubsystem();

    await subsystem.handleSetAgentGoalObjectiveRequest({
    ...
    expect(operations.goalCalls).toEqual([{ agentId: "agent-1", objective: "Ship the goal bar" }]);
    expect(emitted).toEqual([
      {
        type: "set_agent_goal_objective_response",
        payload: { requestId: "req-1", agentId: "agent-1", accepted: true, error: null },
      },
    ]);
  });

  test("control goal: forwards the action and emits an accepted response", async () => {
    const { subsystem, emitted, operations } = makeSubsystem();

    await subsystem.handleControlAgentGoalRequest({
    ...
    expect(operations.goalControlCalls).toEqual([{ agentId: "agent-1", action: "pause" }]);
    expect(emitted).toEqual([
      {
        type: "control_agent_goal_response",
        payload: { requestId: "req-1", agentId: "agent-1", accepted: true, error: null },
      },
    ]);
  });
