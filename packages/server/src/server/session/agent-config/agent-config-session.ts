...
import type { AgentConfigApply } from "@omp-desktop/protocol/messages";
...
export interface AgentConfigOperations {
  ...
  setGoalObjective(agentId: string, objective: string): Promise<void>;
  controlGoal(agentId: string, action: "start" | "pause" | "delete"): Promise<void>;
  setThinking(
    agentId: string,
...
export class AgentConfigSession {
  ...
  handleSetAgentGoalObjectiveRequest(
    msg: Extract<SessionInboundMessage, { type: "set_agent_goal_objective_request" }>,
  ): Promise<void> {
    const { agentId, objective, requestId } = msg;
    return this.applyConfigChange({
      agentId,
      requestId,
      logLabel: "set_agent_goal_objective_request",
      logFields: { agentId, objective, requestId },
      failureText: "Failed to set agent goal objective",
      run: async () => {
        await this.operations.setGoalObjective(agentId, objective);
        return undefined;
      },
      emitResponse: (payload) =>
        this.host.emit({ type: "set_agent_goal_objective_response", payload }),
    });
  }

  handleControlAgentGoalRequest(
    msg: Extract<SessionInboundMessage, { type: "control_agent_goal_request" }>,
  ): Promise<void> {
    const { agentId, action, requestId } = msg;
    return this.applyConfigChange({
      agentId,
      requestId,
      logLabel: "control_agent_goal_request",
      logFields: { agentId, action, requestId },
      failureText: "Failed to control agent goal",
      run: async () => {
        await this.operations.controlGoal(agentId, action);
        return undefined;
      },
      emitResponse: (payload) => this.host.emit({ type: "control_agent_goal_response", payload }),
    });
  }
