...
function matchesAgentStructuralFilter(
  ...
  if (filter.workspaceIds && filter.workspaceIds.length > 0) {
    const workspaceIds = new Set(filter.workspaceIds.map((item) => item.trim()).filter(Boolean));
    if (workspaceIds.size > 0 && (!agent.workspaceId || !workspaceIds.has(agent.workspaceId))) {
      return false;
    }
  }
