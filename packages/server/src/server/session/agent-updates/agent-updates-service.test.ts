...
describe("matchesAgentUpdatesFilter", () => {
  ...
  test("workspaceIds filter, ignoring blank entries", () => {
    const inWorkspace = makeAgentPayload({ id: "a", workspaceId: "workspace-1" });
    const otherWorkspace = makeAgentPayload({ id: "b", workspaceId: "workspace-2" });
    expect(
      matchesAgentUpdatesFilter({
        agent: inWorkspace,
        project,
        filter: { workspaceIds: ["workspace-1"] },
      }),
    ).toBe(true);
    expect(
      matchesAgentUpdatesFilter({
        agent: otherWorkspace,
        project,
        filter: { workspaceIds: ["workspace-1"] },
      }),
    ).toBe(false);
