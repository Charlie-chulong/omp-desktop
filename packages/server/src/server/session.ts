...
import {
  ...
  stopAgentTerminalProcess,
  stopAllAgentTerminalProcesses,
...
import type { TerminalActivity } from "@omp-desktop/protocol/terminal-activity";
import type { BinaryFrame } from "@omp-desktop/protocol/binary-frames/index";
...
import { getErrorMessage, getErrorMessageOr } from "@omp-desktop/protocol/error-utils";
import { getAgentStatusPriority } from "@omp-desktop/protocol/agent-state-bucket";
...
export interface SessionOptions {
  ...
  pluginRuntime?: {
    listPlugins(): import("@omp-desktop/protocol/messages").PluginListItem[];
    getLogs(pluginId: string): import("@omp-desktop/protocol/messages").PluginLogEntry[];
    installDirectory(input: {
      path: string;
      id?: string;
    }): Promise<import("@omp-desktop/protocol/messages").PluginListItem>;
...
export class Session {
  ...
  constructor(options: SessionOptions) {
    ...
    this.agentConfigSession = new AgentConfigSession({
      ...
      operations: {
        ...
        setGoalObjective: (agentId, objective) =>
          agentManager.setAgentGoalObjective(agentId, objective),
        controlGoal: (agentId, action) => agentManager.controlAgentGoal(agentId, action),
  ...
  private dispatchAgentTimelineMessage(
    ...
    switch (msg.type) {
      ...
      case "agent.background_processes.stop.request":
  ...
  private dispatchAgentConfigMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      ...
      case "set_agent_goal_objective_request":
        return this.agentConfigSession.handleSetAgentGoalObjectiveRequest(msg);
      case "control_agent_goal_request":
        return this.agentConfigSession.handleControlAgentGoalRequest(msg);
  ...
  private dispatchWorkspaceAndProjectMessage(
    ...
    switch (msg.type) {
      ...
      case "delete_workspace_request":
        return this.handleDeleteWorkspaceRequest(msg);
  ...
  private dispatchOmpProviderMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      ...
      case "omp.install.request":
        return this.providerCatalogSession.handleOmpInstallRequest(msg);
      case "omp.provider.login.start.request":
  ...
  private async handleCancelAgentRequest(agentId: string, requestId?: string): Promise<void> {
    ...
    try {
      ...
      if (this.terminalManager) {
        await stopAllAgentTerminalProcesses(this.terminalManager, agentId);
      }
      if (requestId) {
  ...
  private async handleDirectoryExistsRequest(
    msg: Extract<SessionInboundMessage, { type: "directory_exists_request" }>,
  ): Promise<void> {
    ...
    try {
      const stats = await stat(expandTilde(cwd));
      this.emit({
        type: "directory_exists_response",
        payload: {
          cwd,
  ...
  private async handleDeleteWorkspaceRequest(
    request: Extract<SessionInboundMessage, { type: "delete_workspace_request" }>,
  ): Promise<void> {
    try {
      const existing = await this.workspaceRegistry.get(request.workspaceId);
      if (!existing) {
      ...
      for (const agentId of result.archivedAgentIds) {
        await this.agentStorage.remove(agentId);
        await this.agentManager.deleteAgentState(agentId);
        await this.agentUpdates.removeAgent(agentId);
      }
      await this.workspaceRegistry.remove(existing.workspaceId);
      await this.emitWorkspaceUpdatesForWorkspaceIds([existing.workspaceId]);
  ...
  private async handleBackgroundProcessRequest(
    msg: Extract<
      SessionInboundMessage,
      {
        type:
          ...
          | "agent.background_processes.stop.request";
      }
    >,
    source?: object,
  ): Promise<void> {
    try {
      ...
      if (msg.type === "agent.background_processes.list.request") {
      ...
      } else if (msg.type === "agent.background_processes.output.request") {
        const output =
        ...
        const stopped =
          msg.processId.startsWith("terminal:") && this.terminalManager
            ? await stopAgentTerminalProcess(this.terminalManager, msg.agentId, msg.processId)
            : await this.agentManager.stopBackgroundProcess(msg.agentId, msg.processId);
        this.emitForSource(
          {
            type: "agent.background_processes.stop.response",
            payload: { requestId: msg.requestId, processId: msg.processId, stopped, error: null },
          },
          source,
        );
      }
