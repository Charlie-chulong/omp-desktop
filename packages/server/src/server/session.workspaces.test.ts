...
import type { AgentSnapshotPayload, SessionOutboundMessage } from "@omp-desktop/protocol/messages";
...
test("delete_workspace_request permanently removes the workspace record", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const session = createSessionForWorkspaceTests();
  const workspace = createPersistedWorkspaceRecord({
    workspaceId: "ws-repo-delete",
    projectId: "proj-repo-delete",
    cwd: REPO_CWD,
    kind: "directory",
    displayName: "repo",
    createdAt: "2026-03-01T12:00:00.000Z",
  ...
  session.workspaceRegistry.get = async () => (removed ? null : workspace);
  session.workspaceRegistry.archive = async (_workspaceId: string, archivedAt: string) => {
    workspace.archivedAt = archivedAt;
  };
  session.workspaceRegistry.remove = async () => {
    removed = true;
  };
  session.workspaceRegistry.list = async () => (removed ? [] : [workspace]);
  session.projectRegistry.archive = async () => {};
  ...
  const response = findByType(emitted, "delete_workspace_response");
