...
import type { ProjectCheckoutLitePayload } from "@omp-desktop/protocol/messages";
      ...
      const subscription = await this.subscribeWithDeadline(
        ...
        (error, events) => {
          ...
          if (relevantEvents.length > 0) {
            this.refreshWorkingTreeIgnoresFromRepoMetadataEvents(target, relevantEvents);
            const routedRefreshes = this.routeRepoMetadataEvents(target, relevantEvents);
