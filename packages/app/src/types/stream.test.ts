...
import { buildToolCallDisplayModel } from "@omp-desktop/protocol/tool-call-display";
...
describe("stream reducer canonical tool calls", () => {
  ...
  it("keeps a plan in one semantic card instead of promoting its markdown blocks", () => {
    const result = applyStreamEvent({
      tail: [],
      head: [],
...
describe("turn lifecycle events", () => {
  ...
  it("merges an idless canonical echo into a recent submitted image message", () => {
    const submittedTimestamp = new Date("2025-01-01T15:03:10Z");
    const image = {
      id: "image-idless-canonical",
      mimeType: "image/png",
      storageType: "web-indexeddb" as const,
      storageKey: "image-idless-canonical",
      createdAt: submittedTimestamp.getTime(),
    };
    const submitted = createUserMessage({
      clientMessageId: "msg_submitted_idless",
    ...
    const userMessage = userMessages[0];
    invariant(userMessage?.kind === "user_message");
    assert.strictEqual(userMessage.id, "msg_submitted_idless");
    assert.deepStrictEqual(userMessage.images, [image]);
    assert.deepStrictEqual(userMessage.timelineCursor, { epoch: "epoch-1", seq: 43 });
  });

  it("merges an idless canonical echo into a recent submitted plain-text message", () => {
    const submitted = createUserMessage({
      clientMessageId: "msg_plain_idless",
      text: "Package it directly",
      timestamp: new Date("2025-01-01T15:04:00Z"),
    });
    const state = reduceStreamUpdate(
      [submitted],
      {
        type: "timeline",
        provider: "omp",
        item: { type: "user_message", text: "Package it directly" },
      },
      new Date("2025-01-01T15:04:01Z"),
      {
        source: "canonical",
        timelineCursor: { epoch: "epoch-1", seq: 44 },
      },
    );

    const userMessages = state.filter((item) => item.kind === "user_message");
    assert.strictEqual(userMessages.length, 1);
    assert.strictEqual(userMessages[0]?.id, "msg_plain_idless");
    assert.deepStrictEqual(userMessages[0]?.timelineCursor, { epoch: "epoch-1", seq: 44 });
  });

  it("keeps consecutive identical submissions distinct while reconciling idless echoes", () => {
    const first = createUserMessage({
      clientMessageId: "msg_same_first",
      text: "Continue",
      timestamp: new Date("2025-01-01T15:05:00Z"),
    });
    const second = createUserMessage({
      clientMessageId: "msg_same_second",
      text: "Continue",
      timestamp: new Date("2025-01-01T15:05:02Z"),
    });
    const firstCanonical = reduceStreamUpdate(
      [first, second],
      {
        type: "timeline",
        provider: "omp",
      ...
      new Date("2025-01-01T15:05:01Z"),
      {
        source: "canonical",
        timelineCursor: { epoch: "epoch-1", seq: 45 },
      },
    );
    const secondCanonical = reduceStreamUpdate(
      firstCanonical,
      {
        type: "timeline",
        provider: "omp",
      ...
      new Date("2025-01-01T15:05:03Z"),
      {
        source: "canonical",
        timelineCursor: { epoch: "epoch-1", seq: 46 },
      },
    );

    const userMessages = secondCanonical.filter((item) => item.kind === "user_message");
    assert.deepStrictEqual(
      userMessages.map((message) => [message.id, message.timelineCursor?.seq]),
      [
        ["msg_same_first", 45],
        ["msg_same_second", 46],
      ],
