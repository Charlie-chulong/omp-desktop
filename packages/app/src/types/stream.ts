...
function matchesIdlessCanonicalUserMessage(
  submitted: UserMessageItem,
  canonical: UserMessageItem,
): boolean {
  if (!isUnreconciledLocalUserMessage(submitted)) return false;
  if (canonical.clientMessageId !== undefined || canonical.messageId !== undefined) return false;
  return (
    submitted.text === canonical.text &&
    Math.abs(submitted.timestamp.getTime() - canonical.timestamp.getTime()) <= 2 * 60 * 1000
  );
}
...
function matchesUserMessage(
  ...
  if (matchesIdlessCanonicalUserMessage(existing, incoming)) return true;
...
function isMatchingAssistantStream(
  item: StreamItem | undefined,
  messageId: string | undefined,
  presentation: "plan" | undefined,
): item is AssistantMessageItem {
  if (item?.kind !== "assistant_message" || item.presentation !== presentation) {
    return false;
  }
  return messageId === undefined || item.messageId === messageId;
}
...
function appendAssistantMessage(
  ...
  const shouldAppendToLast = isMatchingAssistantStream(last, messageId, presentation);
  ...
  if (
    ...
    isMatchingAssistantStream(secondLast, messageId, presentation)
  ) {
    const updated: AssistantMessageItem = {
      ...secondLast,
...
function promoteCompletedAssistantBlocks(params: { tail: StreamItem[]; head: StreamItem[] }): {
  ...
  if (
    assistantIndex < 0 ||
    !activeItem ||
    activeItem.kind !== "assistant_message" ||
    activeItem.presentation === "plan"
  ) {
    return {
      tail: params.tail,
      head: params.head,
