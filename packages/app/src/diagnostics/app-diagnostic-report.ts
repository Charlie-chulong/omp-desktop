import type { ServerInfoStatusPayload } from "@omp-desktop/protocol/messages";
...
export function formatAppDiagnosticHeader(input: {
  ...
  return formatDiagnosticSection("OMP Desktop app diagnostics", [
...
export function redactAppDiagnosticReport(report: string, hosts: HostProfile[]): string {
  ...
  return redacted
    .replace(/omp-desktop:\/\/\S+/gi, "omp-desktop://[redacted]")
    .replace(
