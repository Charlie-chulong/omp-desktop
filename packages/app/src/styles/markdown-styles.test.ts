...
import { darkTheme, lightTheme, ompBrandColors } from "./theme";
...
describe("createMarkdownStyles", () => {
  ...
  it("uses the OMP cyan highlight for markdown links", () => {
    const darkStyles = createMarkdownStyles(darkTheme);
    const lightStyles = createMarkdownStyles(lightTheme);

    expect(darkStyles.link.color).toBe(ompBrandColors.cyan);
    expect(darkStyles.blocklink.color).toBe(ompBrandColors.cyan);
    expect(lightStyles.link.color).toBe(ompBrandColors.cyanOnLight);
    expect(lightStyles.blocklink.color).toBe(ompBrandColors.cyanOnLight);
  });
