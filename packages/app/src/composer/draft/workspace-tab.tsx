...
export function WorkspaceDraftAgentTab({
  ...
  const saveDraftGoal = useCallback(
    async (objective: string) => {
      draftOnSetFeature?.("goal_objective", objective);
    },
    [draftOnSetFeature],
  );
  const controlDraftGoal = useCallback(
    async (action: GoalControlAction) => {
      if (action === "delete") {
        draftOnSetFeature?.("goal_objective", "");
        draftOnSetFeature?.("goal_status", "paused");
        draftOnSetFeature?.("workflow_mode", "standard");
        return;
      }
      draftOnSetFeature?.("goal_status", action === "start" ? "active" : "paused");
    },
    [draftOnSetFeature],
  );
  return (
