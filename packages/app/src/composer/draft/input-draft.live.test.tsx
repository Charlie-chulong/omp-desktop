...
describe("useAgentInputDraft live contract", () => {
  ...
  it("attaches a directory to an unmounted draft", async () => {
    useDraftStore.getState().saveDraftInput({
      draftKey: "draft:directory",
      draft: { text: "inspect this", attachments: [] },
    });

    const attachment = createDirectoryComposerAttachment("/Users/alice/project/src");
    await useDraftStore.getState().attachDirectory({
      draftKey: "draft:directory",
      attachment,
    });

    expect(useDraftStore.getState().getDraftInput("draft:directory")).toEqual({
      text: "inspect this",
