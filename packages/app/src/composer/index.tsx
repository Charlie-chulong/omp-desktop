...
import { assertWorkspaceDirectoryExists, submitAgentInput } from "@/composer/submit";
...
import type {
  ...
  QuotedContentComposerAttachment,
  WorkspaceComposerAttachment,
} from "@/attachments/types";
import {
  appendDirectoryAttachment,
  createDirectoryComposerAttachment,
  getDirectoryAttachmentKey,
  getDirectoryAttachmentSubtitle,
  getDirectoryName,
...
function renderComposerAttachmentPill(args: RenderComposerAttachmentPillArgs): ReactElement {
  ...
  if (attachment.kind === "directory") {
    return (
      <DirectoryAttachmentPill
        key={`directory:${getDirectoryAttachmentKey(attachment)}`}
        attachment={attachment}
        index={index}
        disabled={disabled}
        onRemove={onRemove}
        directoryLabel={labels.directory}
        removeLabel={labels.removeDirectory}
      />
    );
  }
  ...
  if (attachment.kind === "quoted_content") {
    return (
      <QuotedContentAttachmentPill
        key={attachment.id}
        attachment={attachment}
        index={index}
        disabled={disabled}
...
interface DirectoryAttachmentPillProps {
  attachment: Extract<ComposerAttachment, { kind: "directory" }>;
  index: number;
  disabled: boolean;
  onRemove: (index: number) => void;
...
function DirectoryAttachmentPill({
  attachment,
  index,
  disabled,
  onRemove,
  directoryLabel,
  removeLabel,
}: DirectoryAttachmentPillProps) {
  const handleRemove = useCallback(() => {
    onRemove(index);
  }, [index, onRemove]);
  const directoryName = getDirectoryName(attachment.path);
  return (
    <AttachmentPill
      testID="composer-directory-attachment-pill"
      onOpen={noopCallback}
      onRemove={handleRemove}
      openAccessibilityLabel={directoryName}
      removeAccessibilityLabel={removeLabel}
      disabled={disabled}
    >
      <AttachmentLabel
        icon={directoryPillIcon}
        title={directoryName}
        subtitle={getDirectoryAttachmentSubtitle(attachment) || directoryLabel}
      />
    </AttachmentPill>
  );
}
...
interface QuotedContentAttachmentPillProps {
  attachment: QuotedContentComposerAttachment;
  index: number;
  disabled: boolean;
  onRemove: (index: number) => void;
}

function QuotedContentAttachmentPill({
  attachment,
  index,
  disabled,
  onRemove,
}: QuotedContentAttachmentPillProps) {
  const { t } = useTranslation();
  const handleRemove = useCallback(() => onRemove(index), [index, onRemove]);
  return (
    <AttachmentPill
      testID="composer-quoted-content-attachment-pill"
      onOpen={noopCallback}
      onRemove={handleRemove}
      openAccessibilityLabel={t("composer.attachments.quotedContent")}
      removeAccessibilityLabel={t("composer.attachments.removeQuotedContent")}
      disabled={disabled}
    >
      <AttachmentLabel
        icon={quotedContentPillIcon}
        title={t("composer.attachments.quotedContent")}
...
function ComposerCancelButton({
  ...
  const icon = isCancellingAgent ? (
    <ThemedLoadingSpinner size="small" uniProps={iconForegroundMapping} />
  ) : (
    <ThemedSquare size={buttonIconSize} uniProps={filledIconForegroundMapping} />
  );
...
function ComposerContentImpl({
  ...
  const addDirectory = useCallback(
    (path: string) => {
      const attachment = createDirectoryComposerAttachment(path);
      setSelectedAttachments((prev) => appendDirectoryAttachment(prev, attachment));
    },
    [setSelectedAttachments],
  );
  ...
  const submitMessage = useCallback(
    async (text: string, submitAttachments: ComposerAttachment[]) => {
      if (client) {
        await assertWorkspaceDirectoryExists({
          cwd,
          directoryExists: (path) => client.directoryExists(path),
          missingDirectoryMessage: t("panels.file.directoryMissing"),
        });
      }
      onMessageSent?.();
    ...
    [appSettings.sendBehavior, client, cwd, onMessageSent, t],
  );
  ...
  const canPickDirectory = isElectronRuntime() && isLocalDaemon;
  const handlePickDirectory = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Composer] Failed to pick directory:", error);
      toastErrorRef.current(
        error instanceof Error ? error.message : t("composer.errors.directoryPickFailed"),
      );
    }
    ...
    async (items: DroppedItem[]) => {
      try {
        const directoryPaths = items
          .filter((item) => item.kind === "directory-path")
          .map((item) => item.path);
        if (directoryPaths.length > 0) {
          if (!canPickDirectory) {
            toastErrorRef.current(t("composer.errors.directoryRequiresLocalDaemon"));
          } else {
  ...
  const attachmentMenuItems = useMemo<AttachmentMenuItem[]>(() => {
    ...
    if (canPickDirectory) {
      items.push({
        id: "directory",
        label: t("composer.attachments.addDirectory"),
        icon: <ThemedFolder size={ICON_SIZE.md} uniProps={iconForegroundMutedMapping} />,
        onSelect: () => {
    ...
    () =>
      renderAttachmentTray({
        ...
        labels: {
          ...
          directory: t("message.attachments.directory"),
          removeDirectory: t("composer.attachments.removeDirectory"),
...
const ThemedQuote = withUnistyles(Quote);
const iconForegroundMapping = (theme: Theme) => ({ color: theme.colors.foreground });
const iconForegroundMutedMapping = (theme: Theme) => ({ color: theme.colors.foregroundMuted });
const iconAccentForegroundMapping = (theme: Theme) => ({ color: theme.colors.accentForeground });
const filledIconForegroundMapping = (theme: Theme) => ({
  color: theme.colors.foreground,
...
const quotedContentPillIcon = (
  <ThemedQuote size={ICON_SIZE.sm} uniProps={iconForegroundMutedMapping} />
);
const directoryPillIcon = (
  <ThemedFolder size={ICON_SIZE.sm} uniProps={iconForegroundMutedMapping} />
);
