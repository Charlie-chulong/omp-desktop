...
describe("ReplicaCache", () => {
  ...
  it("persists the complete directory with only the focused timeline tail", async () => {
    ...
    const persisted = JSON.parse(storage.values.get("@omp-desktop:replica-cache") ?? "null") as {
      version: number;
  ...
  it("rejects and clears version 5 cache data before overwriting it on flush", async () => {
    ...
    await cache.restore();
    expect(storage.values.has("@omp-desktop:replica-cache")).toBe(false);
    await cache.flush();

    expect(useSessionStore.getState().sessions[SERVER_ID]).toBeUndefined();
    expect(JSON.parse(storage.values.get("@omp-desktop:replica-cache") ?? "null")).toEqual({
      version: 6,
      hosts: [],
    });
  });
