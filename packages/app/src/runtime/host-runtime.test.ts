...
import type { AgentPermissionRequest } from "@omp-desktop/protocol/agent-types";
...
describe("HostRuntimeStore", () => {
  it("revokes push notifications before removing a host", async () => {
    ...
    const storage = createMemoryHostRuntimeStorage({
      "@omp-desktop:daemon-registry": JSON.stringify([host]),
      "@omp-desktop:e2e": "1",
    });
  ...
  it("restores the display replica before declaring the host registry loaded", async () => {
    const host = makeHost();
    const storage = createMemoryHostRuntimeStorage();
    await storage.setItem("@omp-desktop:daemon-registry", JSON.stringify([host]));
    await storage.setItem("@omp-desktop:e2e", "1");
  ...
  it("records a chosen host color and writes it through to storage", async () => {
    ...
    const persisted = await storage.getItem("@omp-desktop:daemon-registry");
  ...
  it("serializes overlapping host appearance writes", async () => {
    ...
    const persistedHosts = JSON.parse(
      (await storage.getItem("@omp-desktop:daemon-registry")) ?? "[]",
    );
