...
import { QuotedSelectionContextMenu } from "@/components/quoted-selection-context-menu";
...
const ChatAgentReadyContent = memo(function ChatAgentReadyContent({
  ...
  const goalModeEnabled =
    agentState.features?.some(
      (feature) =>
        feature.type === "select" && feature.id === "workflow_mode" && feature.value === "goal",
    ) === true;
  const goal = useStoreWithEqualityFn(
    useSessionStore,
    (state) => {
      const session = state.sessions[serverId];
      return findLatestGoalSnapshot(
  ...
  const saveGoalObjective = useCallback(
    async (objective: string) => {
      if (!client) {
        throw new Error(t("common.errors.daemonClientUnavailable"));
      }
      try {
        await client.setAgentGoalObjective(agentId, objective);
      } catch (error) {
        toastApi.error(error instanceof Error ? error.message : t("common.errors.unableToSave"));
        throw error;
      }
    },
  ...
  const controlGoal = useCallback(
    async (action: "start" | "pause" | "delete") => {
      ...
      try {
        await client.controlAgentGoal(agentId, action);
      } catch (error) {
        toastApi.error(error instanceof Error ? error.message : t("common.errors.unableToSave"));
        throw error;
      }
  ...
  const handleQuoteSelection = useCallback(
    (quotedText: string) => {
      agentInputDraft.setAttachments([
        ...agentInputDraft.attachments,
        {
          kind: "quoted_content",
