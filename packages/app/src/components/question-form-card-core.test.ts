...
describe("question form card core", () => {
  ...
  test("localizes standard approval options without changing custom labels", () => {
    const localized = { approve: "批准", deny: "拒绝" };

    expect(resolveQuestionOptionLabel("Approve", localized)).toBe("批准");
    expect(resolveQuestionOptionLabel("Deny", localized)).toBe("拒绝");
    expect(resolveQuestionOptionLabel("Always allow", localized)).toBe("Always allow");
  });
