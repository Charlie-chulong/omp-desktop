...
describe("agent deep links", () => {
  it("round-trips an existing agent target", () => {
    ...
    expect(link).toBe("omp-desktop://h/server%2Fmain/agent/agent%20123");
  ...
  it("rejects links outside the exact agent route", () => {
    ...
    expect(parseAgentDeepLink("omp-desktop://app/h/server/agent/agent-1")).toBeNull();
    expect(parseAgentDeepLink("omp-desktop://h/server/agent/agent-1?message=hello")).toBeNull();
    expect(parseAgentDeepLink("omp-desktop://h/server/agent/agent-1/extra")).toBeNull();
  });
});
