...
import type { AgentAttentionNotificationPayload } from "@omp-desktop/protocol/agent-attention-notification";
import {
...
import { validateWSOutboundMessage } from "@omp-desktop/protocol/validation/ws-outbound";
import type {
  ...
  DeleteWorkspaceResponseMessage,
...
import { terminalSubscriptionKey } from "@omp-desktop/protocol/terminal-subscription-key";
import {
  asUint8Array,
...
type DeleteWorkspacePayload = DeleteWorkspaceResponseMessage["payload"];
...
export class DaemonClient {
  ...
  async deleteWorkspace(workspaceId: string, requestId?: string): Promise<DeleteWorkspacePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId,
      message: {
      ...
      responseType: "delete_workspace_response",
    });
  }
  ...
  async setAgentGoalObjective(agentId: string, objective: string): Promise<void> {
    const requestId = this.createRequestId();
    const message = SessionInboundMessageSchema.parse({
      type: "set_agent_goal_objective_request",
      agentId,
      objective,
      requestId,
    });
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "set_agent_goal_objective_response") {
          return null;
        }
        if (msg.payload.requestId !== requestId) {
          return null;
    ...
    if (!payload.accepted) {
      throw new Error(payload.error ?? "setAgentGoalObjective rejected");
    }
  }

  async controlAgentGoal(agentId: string, action: "start" | "pause" | "delete"): Promise<void> {
    const requestId = this.createRequestId();
    const message = SessionInboundMessageSchema.parse({
      type: "control_agent_goal_request",
      agentId,
      action,
      requestId,
    });
    const payload = await this.sendRequest({
      ...
      options: { skipQueue: true },
      select: (msg) => {
        if (msg.type !== "control_agent_goal_response") return null;
        if (msg.payload.requestId !== requestId) return null;
        return msg.payload;
      },
    });
    if (!payload.accepted) {
      throw new Error(payload.error ?? "controlAgentGoal rejected");
    }
  }
  ...
  async directoryExists(cwd: string, requestId?: string): Promise<boolean> {
    const payload: DirectoryExistsPayload = await this.sendCorrelatedSessionRequest({
      requestId,
      message: {
        type: "directory_exists_request",
        cwd,
      },
      responseType: "directory_exists_response",
    });
    if (payload.error) {
      throw new Error(payload.error);
    }
  ...
  async getOmpProviderManagement(options?: {
    requestId?: string;
  }): Promise<OmpProviderManagementGetPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
  ...
  async saveOmpProviderConfig(
    configYaml: string,
    options?: { requestId?: string },
  ): Promise<OmpProviderManagementSavePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
  ...
  async addOmpProvider(
    ...
    options?: { requestId?: string },
  ): Promise<OmpProviderManagementAddPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
      message: { type: "omp.provider.management.add.request", provider },
      responseType: "omp.provider.management.add.response",
      timeout: 180000,
    });
  }
  async discoverOmpProviderModels(
    input: { baseUrl: string; apiKey: string },
    options?: { requestId?: string },
  ): Promise<OmpProviderModelDiscoveryPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
      message: { type: "omp.provider.models.discover.request", ...input },
      responseType: "omp.provider.models.discover.response",
      timeout: 30_000,
    });
  }

  async removeOmpProvider(
    providerId: string,
    options?: { requestId?: string },
  ): Promise<OmpProviderManagementRemovePayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
      message: { type: "omp.provider.management.remove.request", providerId },
      responseType: "omp.provider.management.remove.response",
      timeout: 180000,
    });
  }
  ...
  }): Promise<OmpInstallStatusPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
      message: {
        type: "omp.install.status.request",
        checkForUpdates: options?.checkForUpdates,
      },
      responseType: "omp.install.status.response",
      timeout: 90_000,
    });
  }
  ...
  async startOmpProviderLogin(
    ...
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
      message: { type: "omp.provider.login.start.request", providerId },
      responseType: "omp.provider.login.start.response",
      timeout: 180000,
    });
  }
  ...
  async finishOmpProviderLogin(
    ...
    input?: string,
    options?: { requestId?: string },
  ): Promise<OmpProviderLoginFinishPayload> {
    return this.sendCorrelatedSessionRequest({
      requestId: options?.requestId,
      message: {
        type: "omp.provider.login.finish.request",
        flowId,
        ...(input ? { input } : {}),
      },
