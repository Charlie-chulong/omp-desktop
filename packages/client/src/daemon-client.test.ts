...
} from "@omp-desktop/protocol/terminal-stream-protocol";
...
test("checks whether a project directory exists", async () => {
  const mock = createMockTransport();
  const client = new DaemonClient({
    url: "ws://test",
  ...
  const existsPromise = client.directoryExists("/tmp/project");
