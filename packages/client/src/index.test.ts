...
test("OMP provider actions use the native management RPCs", async () => {
  const { client, ws } = await connectClient({ ompProviderManagement: true });
  const managementPayload = {
    configPath: "/home/test/.omp/agent/models.yml",
    configYaml: "providers: {}\n",
    providerModels: [{ id: "openai", modelCount: 12 }],
    loginProviders: [{ id: "openai", name: "OpenAI", available: true, authenticated: false }],
  };

  const getPromise = client.omp.getProviderManagement({ requestId: "omp-management-get" });
  expect(parseSentFrame(ws.sent.at(-1))).toMatchObject({
    type: "session",
    message: {
      type: "omp.provider.management.get.request",
      requestId: "omp-management-get",
    },
  });
  ws.message(
    sessionMessage({
      type: "omp.provider.management.get.response",
      payload: { ...managementPayload, requestId: "omp-management-get" },
    }),
  );
  await expect(getPromise).resolves.toEqual({
    ...managementPayload,
  ...
  const savePromise = client.omp.saveProviderConfig("providers: {}\n", {
    requestId: "omp-management-save",
  });
  expect(parseSentFrame(ws.sent.at(-1))).toMatchObject({
    type: "session",
    message: {
      type: "omp.provider.management.save.request",
      configYaml: "providers: {}\n",
      requestId: "omp-management-save",
    },
  });
  ws.message(
    sessionMessage({
      ...
      payload: { ...managementPayload, requestId: "omp-management-save" },
    }),
  );
  ...
  const addPromise = client.omp.addProvider(
    {
      ...
      models: [{ id: "gpt-test" }, { id: "gpt-test-mini" }],
    },
    { requestId: "omp-management-add" },
  );
  expect(parseSentFrame(ws.sent.at(-1))).toMatchObject({
    type: "session",
    message: {
      type: "omp.provider.management.add.request",
      provider: {
        providerId: "mintcat",
        models: [{ id: "gpt-test" }, { id: "gpt-test-mini" }],
      },
  ...
  ws.message(
    sessionMessage({
      ...
      payload: { ...managementPayload, requestId: "omp-management-add" },
    }),
  );
  await expect(addPromise).resolves.toMatchObject({ requestId: "omp-management-add" });

  const removePromise = client.omp.removeProvider("mintcat", {
    requestId: "omp-management-remove",
  });
  expect(parseSentFrame(ws.sent.at(-1))).toMatchObject({
    type: "session",
    message: {
      type: "omp.provider.management.remove.request",
      providerId: "mintcat",
      requestId: "omp-management-remove",
    },
  });
  ws.message(
    sessionMessage({
      ...
      payload: { ...managementPayload, requestId: "omp-management-remove" },
    }),
  );
  await expect(removePromise).resolves.toMatchObject({ requestId: "omp-management-remove" });
  ...
  const installationPayload = {
    ...
    installed: true,
    version: "omp 16.3.9",
    installPath: "/Users/test/.local/bin/omp",
  };
  const statusPromise = client.omp.getInstallationStatus({
    requestId: "omp-install-status",
  ...
  ws.message(
    sessionMessage({
      ...
      payload: { ...installationPayload, requestId: "omp-install-status" },
    }),
  );
  ...
  ws.message(
    sessionMessage({
      ...
      payload: { ...installationPayload, requestId: "omp-install" },
    }),
  );
  ...
  await expect(startPromise).resolves.toMatchObject({ flowId: "flow-openai" });

  const finishPromise = client.omp.finishProviderLogin("flow-openai", "oauth-code", {
    requestId: "omp-login-finish",
  });
  expect(parseSentFrame(ws.sent.at(-1))).toMatchObject({
    type: "session",
    message: {
  ...
  ws.message(
    sessionMessage({
      ...
      payload: { ...managementPayload, requestId: "omp-login-finish" },
    }),
  );
