...
export interface OmpProviderActions {
  ...
  addProvider(
    ...
    options?: { requestId?: string },
  ): Promise<OmpProviderManagementAddResponseMessage["payload"]>;
  removeProvider(
    providerId: string,
    options?: { requestId?: string },
  ): Promise<OmpProviderManagementRemoveResponseMessage["payload"]>;
  ...
  startProviderLogin(
    providerId: string,
    options?: { requestId?: string },
  ): Promise<OmpProviderLoginStartResponseMessage["payload"]>;
  finishProviderLogin(
    flowId: string,
    input?: string,
    options?: { requestId?: string },
  ...
  return {
    workspaces: {
      ...
      archive: (workspace, requestId) =>
        daemonClient.archiveWorkspace(resolveWorkspaceId(workspace), undefined, requestId),
      subscribe: (handler) =>
  ...
  return (workspace) => {
    ...
    return {
      ...
      archive: async (requestId) => {
        const result = await daemonClient.archiveWorkspace(id, undefined, requestId);
        if (current) {
