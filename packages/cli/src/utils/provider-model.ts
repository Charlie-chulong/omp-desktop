...
export function resolveProviderAndModel(
  ...
  const providerInput = options.provider?.trim() || options.defaultProvider || "omp";
  ...
  if (slashIndex === -1) {
    if (providerInput !== "omp") {
      throw {
        code: "INVALID_PROVIDER",
        message: `OMP Desktop supports only the 'omp' provider, received '${providerInput}'`,
  ...
  if (provider !== "omp") {
    throw {
      code: "INVALID_PROVIDER",
      message: `OMP Desktop supports only the 'omp' provider, received '${provider}'`,
    } satisfies CommandError;
  }
