...
const DAEMON_PID_FILENAME = "omp-desktop.pid";
...
function envWithHome(home?: string): NodeJS.ProcessEnv {
  ...
  return { ...process.env, OMP_DESKTOP_HOME: home };
}
...
function buildChildEnv(options: DaemonStartOptions): NodeJS.ProcessEnv {
  ...
  if (options.home) {
    childEnv.OMP_DESKTOP_HOME = options.home;
  }
  if (options.listen) {
...
function resolveServerRunnerFromDir(currentDir: string): string | null {
  ...
  try {
    ...
    if (packageJson.name !== "@omp-desktop/server") return null;
...
function resolveDaemonRunnerEntry(): string {
  const serverExportPath = require.resolve("@omp-desktop/server");
  ...
  throw new Error("Unable to resolve @omp-desktop/server package root for daemon runner");
}
