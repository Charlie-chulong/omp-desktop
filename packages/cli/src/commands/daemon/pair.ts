...
async function resolveDaemonPairingOffer(
  ...
  try {
    ...
    if (serverInfo?.serverId.trim() !== expectedServerId) {
      throw new Error(
        "The reachable daemon belongs to a different OMP Desktop home. Check --home or the daemon listen configuration.",
      );
    }
    if (serverInfo?.features?.daemonStatusRpc !== true) {
      throw new Error("Update the OMP Desktop daemon before pairing from this command.");
    }
...
export async function runPairCommand(
  ...
  if (options.home) process.env.OMP_DESKTOP_HOME = options.home;
    ...
    if (options.json) {
      output.writeStderr(
        `${JSON.stringify({
          ...
          action: "Run omp-desktop daemon pair --relay --json to enable it explicitly.",
        })}\n`,
      );
    } else {
      output.writeStderr(`${chalk.red("Relay pairing is disabled for this daemon.")}\n`);
      output.writeStderr(`${chalk.yellow("Run omp-desktop daemon pair --relay to enable it.")}\n`);
    }
