...
export function addImportOptions(cmd: Command): Command {
  return cmd
    .description("Import an existing OMP session as an OMP Desktop agent")
    .argument("<id>", "OMP session ID to import")
    .option("--cwd <path>", "Working directory for the OMP session")
    .option(
      "--label <key=value>",
