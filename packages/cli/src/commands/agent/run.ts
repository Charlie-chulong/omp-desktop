...
import { getStructuredAgentResponse, StructuredAgentResponseError } from "@omp-desktop/server";
import type { AgentSnapshotPayload } from "@omp-desktop/protocol/messages";
...
export function addRunOptions(cmd: Command): Command {
  return (
    cmd
      ...
      .option("--provider <provider>", "OMP model selection (omp or omp/<model>)")
      .option("--model <model>", "OMP model ID (for example anthropic/claude-sonnet-4-6)")
      .option("--thinking <id>", "Thinking option ID to use for this run")
      .option("--mode <mode>", "OMP permission mode: ask, write, or full")
...
function validateRunWorkspaceOptions(options: AgentRunOptions): void {
  ...
  if (hasWorktreeCreationOptions && newWorkspace !== "worktree") {
    throw {
      ...
      details: "Usage: omp-desktop run --new-workspace worktree [worktree options] <prompt>",
    } satisfies CommandError;
  }
...
function validateRunOptions(prompt: string, options: AgentRunOptions, outputSchema: unknown): void {
  if (!prompt || prompt.trim().length === 0) {
    throw {
      ...
      details: "Usage: omp-desktop agent run [options] <prompt>",
    } satisfies CommandError;
  }
  ...
  try {
    ...
    if (options.thinking !== undefined && !thinkingOptionId) {
      const error: CommandError = {
        ...
        details:
          'Provide a thinking option ID. Use "omp-desktop provider models omp --thinking" to list valid IDs.',
      };
      throw error;
    }
